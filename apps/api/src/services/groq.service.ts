...
class GroqService {
  ...
  async createChatCompletion(messages: ChatMessage[], options?: { model?: string; mode?: string; cortexContext?: string }) {
    ...
    if (!hasLumoraSystem) {
      finalMessages.push({ role: 'system', content: LUMORA_SYSTEM_PROMPT })
    }

    // If caller provided a cortex/SerpAPI context via options, inject it as a system message (sanitized/limited)
    if (options?.cortexContext && typeof options.cortexContext === 'string' && options.cortexContext.trim()) {
      let cortexContent = options.cortexContext.trim()
      // Safety: truncate excessively long external blocks to avoid sending raw dumps
      if (cortexContent.length > 8000) cortexContent = cortexContent.slice(0, 8000) + '\n\n[Truncated external knowledge]'
      finalMessages.push({ role: 'system', content: cortexContent })
    }

    // Append original messages, but avoid duplicating a Lumora system prompt if it already exists
    for (const m of messages) {
      if (m.role === 'system' && typeof m.content === 'string' && m.content.includes('You are the core reasoning engine of Lumora.')) {
        if (!hasLumoraSystem) {
          // move existing Lumora prompt to the front if somehow present but not detected earlier
    ...
    const groqMessages = finalMessages.map((m) => ({ role: m.role === 'assistant' ? 'assistant' : m.role === 'system' ? 'system' : 'user', content: m.content }))
  ...
  async askGroq(opts: { userId: string; message: string; mode?: Mode; conversationId?: string }) {
    ...
    // Build system prompt (centralized Lumora system prompt)
    const systemPrompt = LUMORA_SYSTEM_PROMPT
